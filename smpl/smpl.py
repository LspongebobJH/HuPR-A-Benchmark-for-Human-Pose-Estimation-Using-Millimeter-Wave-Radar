...
class SMPLModel(Module):
	def __init__(self, model_path='./model.pkl'):
		...
		for name in ['J_regressor', 'joint_regressor', 'weights', 'posedirs', 'v_template', 'shapedirs']:
			...
			setattr(self, name, _tensor.cuda())
	...
	def rodrigues(r):
		...
		r_hat = r / theta
		cos = torch.cos(theta)
		z_stick = torch.zeros(theta_dim, dtype=torch.float64).cuda()
		m = torch.stack(
		...
		i_cube = (torch.eye(3, dtype=torch.float64).unsqueeze(dim=0) \
				+ torch.zeros((theta_dim, 3, 3), dtype=torch.float64)).cuda()
	...
	def pack(x):
		...
		"""
		zeros43 = torch.zeros(
			(x.shape[0], x.shape[1], 4, 3), dtype=torch.float64
		).cuda()
	...
	def forward(self, betas, pose, trans, simplify=False):
		...
		if simplify:
			...
			R_cube = R_cube_big[:, 1:, :, :]
			I_cube = (torch.eye(3, dtype=torch.float64).unsqueeze(dim=0) + \
				torch.zeros((batch_num, R_cube.shape[1], 3, 3), dtype=torch.float64)).cuda()
		...
		self.pack(
			torch.matmul(
				stacked,
				torch.reshape(
					torch.cat((J, torch.zeros((batch_num, 24, 1), dtype=torch.float64).cuda()), dim=2),
					(batch_num, 24, 4, 1)
				)
			)
		)
		...
		rest_shape_h = torch.cat(
			(v_posed, torch.ones((batch_num, v_posed.shape[1], 1), dtype=torch.float64).cuda()), dim=2
		)
		...
		joints = torch.matmul(result.transpose(1, 2), self.joint_regressor.transpose(0, 1)).transpose(1, 2)
